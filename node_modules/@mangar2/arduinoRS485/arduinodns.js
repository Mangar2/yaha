...
class ArduinoDNS {
    ...
    _getCommandByTopic (topic) {
        ...
        for (const setting in this._options.settings) {
            if (topic.endsWith(this._options.settings[setting] + '/set')) {
                result = setting
                break
            }
        }
    ...
    _getSerialDataByTopic (mqttMessage) {
        let { topic, value } = mqttMessage
        if (topic.endsWith('/set')) {
            topic = topic.substring(0, topic.length - 4)
        }
        let setting
        if (this._options.topics[topic] !== undefined) {
        ...
        if (types.isString(mqttValue)) {
            for (const interfaceName in this._options.interfaces) {
                ...
                if (arduinoInterface.usedby.includes(command)) {
                    const newValue = arduinoInterface.map[mqttValue.toLowerCase()]
    ...
    toSerialMessage (mqttMessage) {
        ...
        if (this._options.senderAddress < 1 || this._options.senderAddress > 127) {
        ...
        if (data) {
            ...
            serialMessage.value = this._getValue(serialMessage.command, mqttMessage.value)
        }
    ...
    _getTopicBeginUsingSerialAddress (serialAddress) {
        ...
        if (result === undefined) {
            throw Error('Unknown serial address')
        }
        return result
    }
    ...
    _getTopicEndUsingSerialCommand (serialCommand) {
        ...
        if (result === undefined) {
            throw Error('Unknown serial command')
        }
        return result
    }
     ...
     */
    _getMqttMessageUsingSerialMessage (serialMessage) {
        const result = []
