...
class ArduinoDNS {
    ...
    _getCommandByTopic (topic) {
        ...
        if (!topic.endsWith('/set')) {
            throw Error('topics must be set topics ending with "/set"')
        }
    ...
    toMqttMessages (serialMessage, hexString) {
        const result = this._getMqttMessageUsingSerialMessage(serialMessage, hexString)
        if (result.length === 0) {
            const message = new Message(
                this._getTopicUsingSerialMessage(serialMessage),
                serialMessage.value,
                'received from arduino: ' + hexString
            )
            result.push(message)
        }
        return result
