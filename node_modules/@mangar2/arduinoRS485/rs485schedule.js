...
class RS485Schedule {
    ...
    async _sendMessageFromQueue () {
        if (this._sendQueue.hasMessages()) {
            ...
            const isMessageSend = await this._sendMessage(message)
            if (isMessageSend) {
     ...
     * @returns {SerialMessage|null} serial message, if the message should be send to the broker
     */
    processReceivedMessage (messageReceived) {
        ...
        const result = messageReceived.command === TOKEN_COMMAND ? messageReceived : null
        return result
    }
