...
class Automation {
     ...
     * @param {Object} recentMotions information about the recent motions
     * @param {number} recentMotions.timestamp timestamp of the last motion detected
     * @returns {Object} result of rule processing with messages and used variables
     * @memberof Automation
     */
    _processSingleRule (date, rule, recentMotion) {
        let result = { messages: [], usedVariables: {} }
        if (this._isPassedEnoughTimeWithoutMotion(date, recentMotion.timestamp, rule.durationWithoutMovementInMinutes)) {
            let activeMotion = recentMotion.motions
            if (rule.durationWithoutMovementInMinutes === undefined && !this._isMotionListActual(date, recentMotion.timestamp)) {
                activeMotion = {}
            }
            result = this._processRule.check(rule, activeMotion)
        }
        return result
    }
    ...
    processRules (date = new Date(), simulation = false) {
        ...
        for (const rule of this._rules.rules) {
            try {
                const checkResult = this._processSingleRule(date, rule, recentMotion)
