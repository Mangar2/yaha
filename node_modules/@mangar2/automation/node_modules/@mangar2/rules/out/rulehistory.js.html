...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: rulehistory.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: rulehistory.js</h1>
...
class RuleHistory {
    ...
    _isMessageRedundant (rule, date, message) {
        ...
        let isRedundant = historyEntry !== undefined &amp;&amp; historyEntry.value === message.value
        if (isRedundant) {
            if (types.isNumber(rule.cooldownInSeconds)) {
                const cooldown = rule.cooldownInSeconds * MILLISECONDS_IN_A_SECOND
                const needToResend = historyEntry.date.getTime() + cooldown &lt;= date.getTime()
                isRedundant = !needToResend
            } else if (types.isArray(rule.anyOf) || types.isArray(rule.allOf)) {
                // These rules needs a new event to trigger and are thus not redundant
                isRedundant = false
            }
        }
        return isRedundant
    ...
    _updateHistory (name, curDate, topic, value) {
        ...
        this._history[name][topic] = { date, value }
    }
     ...
     * @param {Array} messages list of messages providing new information
     */
    update (rule, date, messages) {
        for (const message of messages) {
            if (!this._isMessageRedundant(rule, date, message)) {
                this._updateHistory(rule.name, date, message.topic, message.value)
            }
        }
