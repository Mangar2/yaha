...
async function saveRulesToFileStore(rules, filestoreOptions) {
    try {
        ...
        const result = await client.post( { path: rules.path, payload: rules, type: 'json' })
...
async function readRulesFromFileStore(filestoreOptions) {
    ...
    try {
        const client = new HttpClient(filestoreOptions.host, filestoreOptions.port)
        const getResult = await client.get(filestoreOptions.path)
        ...
        result = null
        console.log('Could not store rules to filetore, fallback to config files')
        errorLog(err)
    }
    return result
}
