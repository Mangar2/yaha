...
import { Message, TopicMatch } from '@mangar2/mqtt-utils';
...
export class Client {
    ...
    private _will: { qos: number, retain: boolean, topic: string, value: string | number } | undefined;
    public static _maxQueueSize: number | undefined = 100;
    ...
    set port(port: number | undefined) { this._port = port; }
     ...
     * @type {number}
     */
    static set maxQueueSize(size: number | undefined) { Client._maxQueueSize = size; }
    static get maxQueueSize(): number | undefined { return Client._maxQueueSize; }
     ...
     */
    get will(): { qos: number, retain: boolean, topic: string, value: string | number } | undefined { return this._will; }
    set will(will: { qos: number, retain: boolean, topic: string, value: string | number } | undefined) { this._will = will; }
     ...
     */
    restoreFromJSON(jsonObject: Record<string, unknown>): void {
    ...
    /**
     * Recreates the message queue from persisted data.
     */
    private rebuildMessageQueues(): void {
        const rebuildQos0 = new MessageQueue();
        rebuildQos0.rebuild(this.qos0Queue as any);
        this.qos0Queue = rebuildQos0;

        const rebuildQueue = new MessageQueues();
        rebuildQueue.rebuild(this.orderedTopicsQueue as any);
     ...
     */
    connect(clientId: any, host: string, port: number, clean: boolean = true, version: string = '1.0', keepAlive: number = (3600 * 1000))
     ...
     */
    private subscribeOldFormat(subscribe: { QoS: any, topics: string | string[] }): number[] {
     ...
     * @throws {Error} if clientId does not match or subscription is not given
     */
    subscribe(clientId: any, topics: { [key: string]: any }): number[] {
        let result: number[] = [];
        try {
     ...
     * @param {Message} message to add to a publish storage
     * @param {TopicMatch} pattern optional: alternative match filter to filter messages
     */
    publishMessage(message: Message, pattern?: TopicMatch): void {
     ...
     */
    private convertQoSToValidQoS(QoS: any): number {
     ...
     */
    getAllMessagesToSend(inFlightWindow: number, timeoutInMilliseconds: number): Message[] {
        let result: Message[] = [];
