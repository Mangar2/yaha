...
export class MqttSend extends MqttBase {
     ...
     * @param {Connections} connections - mqtt connection manager
     * @param {string} clientId - id of the client or an empty string
     * @returns {HttpReturn} - http return information
     */
    static getConnections(connections: Connections, clientId: string): HttpReturn {
     ...
     */
    private async _sendPublish(connections: Connections, options: MessageQueueEntry): Promise<boolean> {
     ...
     */
    private async _sendPubrel(connections: Connections, options: MessageQueueEntry): Promise<void> {
     ...
     * @private
     */
    private cleanup(connections: Connections): void {
     ...
     */
    async processSendMessage(connections: Connections): Promise<number> {
        const messages = connections.getAllMessagesToSend();
        this.cleanup(connections);
        let promises = [];
        for (const message of messages) {
            let result;
            if (message.isStatusPubrel()) {
                result = this._sendPubrel(connections, message);
            } else {
                result = this._sendPublish(connections, message);
            }
