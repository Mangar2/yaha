...
const { delay, Persist } = require('@mangar2/utils')
...
var Message0 = new Message('topic/0', 'value0', 'reason0', 0)
var Message1 = new Message('topic/1', 'value1', 'reason1', 1)
var Message2 = new Message('topic/2', 'value2', 'reason2', 2)
...
function queueMessages (retain = false) {
    Message0.retain = retain
    Message1.retain = retain
    Message2.retain = retain
    connections.publishMessage(Message0)
    connections.publishMessage(Message1)
    connections.publishMessage(Message2)
}

function getAllMessages (publishExpected, pubrelExpected, errorText, isRetain = false) {
    const messages = connections.getAllMessagesToSend()
    const receivedMessages = { '/publish': 0, '/pubrel': 0 }

    const lastId = {}
    console.log(messages)
    for (const messageQueueEntry of messages) {
        const { host, token, packetid, message, link } = messageQueueEntry
        const { qos, retain, topic } = message

        lastId[host] = lastId[host] || {}
        // check for correct token
        unitTest.assertTrue(token === 'receiveclient0' || token === 'receiveclient1', 'correct token')
        unitTest.assertEqual(retain, isRetain, 'correct retain flag')
...
async function testActionsWhileDisconnected () {
    const text = 'Actions While Disconnected/'
    connect()
    disconnect()
    disconnect()
    const result = subscribe()
    unitTest.assertDeepEqual(result, [0x80], text + 'subscribe while beeing disconnected')
    unitTest.expectException(() => unsubscribe(), text + 'Error', 'unsubscribe beeing disconnected')
    queueMessages()
    getAllMessages(0, 0, text + 'tests beeing disconnected 1')
    connect()
    getAllMessages(2, 0, text + 'tests beeing disconnected 2')
    getAllMessages(0, 1, text + 'tests beeing disconnected 2')
    getAllMessages(0, 0, text + 'tests beeing disconnected 2')
    disconnect()
}
...
async function testSendMessages () {
    ...
    for (const client of ['client0', 'client1']) {
        unitTest.assertEqual(connectResult[client].mqttcode, 0, text + 'connect 1 ' + client)
        unitTest.assertEqual(connectResult[client].present, 0, text + 'connect connection present flag ' + client)
    }
    subscribe()
    queueMessages()
    getAllMessages(6, 0, text + 'test subscribed')
    getAllMessages(0, 1, text + 'test subscribed')
    getAllMessages(0, 0, text + 'test subscribed, messages sent')
}
...
async function testQos2Functions () {
    ...
    await delay(config.pubrelTimeoutInMilliseconds + 100)
    connections.cleanup()
    unitTest.assertFalse(connections.isDuplicateQos2message(...clientAndTopic, 2, true, 1), 'duplicateQos2 ' + id++)
    // Packet 2 is unknown
...
async function testKeepAlive () {
    ...
    for (const wait of [50, 200, 400, 600, 1000]) {
        connect(keepAlive)
        await delay(wait)
        connections.cleanup()
        queueMessages()
        const messageStart = 'keepAlive delay ' + wait + ' '
        if (wait < keepAlive * mqttDisconnectFactor) {
...
async function testWill () {
    ...
    // auto disconnect due to long delay (> keepAlive time)
    await delay(200)
    const disconnected = connections.cleanup()
    unitTest.assertDeepEqual(disconnected,
        {
            client0: { reason: 'inactive', keepAlive: 100 },
            client1: { reason: 'inactive', keepAlive: 100 }
        })
    // Get the will message after connecting
    connect()
    getAllMessages(1, 0, 'will message 1')
    getAllMessages(0, 0, 'will message 2')
    connect(keepAlive)
    disconnect()
    await delay(200)
    connections.cleanup()
    connect()
    // Does not send will messages after disconnect
    getAllMessages(0, 0, 'will message 3')
}
...
function testRetain () {
    ...
    subscribe()
    getAllMessages(6, 0, 'retain, message received due to retain', RETAIN)
    getAllMessages(0, 1, 'retain, message received due to retain', RETAIN)
    disconnect()
    connect()
    getAllMessages(0, 0, 'retain, no messages after reconnect')
    connections.publishMessage({ topic: 'topic/0' }, 0, RETAIN)
...
module.exports = async () => {
    unitTest.expectException(() => {
        const connectWithoutProvidingOptions = new Connections()
        connectWithoutProvidingOptions.cleanup()
    }, Error)
    ...
    return unitTest.getResultFunctions(518)
}
