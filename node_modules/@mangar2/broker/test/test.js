...
let connect = new Connect("/broker/test/client", HOST, PORT, MY_PORT);
...
async function connectToBroker(clean) {
    ...
    let result = await connect.connectAndSubscribe(clean, topics);
    unitTest.assertEqual(result, '{"qos":[0,1,2,1,1]}', "connect");
}

async function disconnectBroker() {
    let result = await connect.disconnect();
...
(async function test() {    
    try {
         ...
         */
        let notConnect = new Connect("hello", HOST, PORT, "morning");
        await notConnect.connect(true).catch(err => { });
        ...
        publish.publish(new Message("/a/0/test1", 1, "test 1"), 0, false);
        await unitTest.delay(DELAY);
        unitTest.assertEqual(receivedTopics["/a/0/test1"].amount, 1, "sending data 0");
        receivedTopics = {};

        for (let i = 1; i <= LAST; i++) {
            publish.publish(new Message("/a/0/test1", i, "test 1"), 0, false);
            publish.publish(new Message("/a/1/test2", i, "test 2"), 1, false);
            publish.publish(new Message("/a/2/test2", i, "test 2"), 1, false);
            publish.publish(new Message("/b/2/test2", i, "test 2"), 1, false);
            publish.publish(new Message("/c/2/1234/z/hello/world", i, "test 2"), 1, false);
            publish.publish(new Message("/d/2/1234/z/hello/world", i, "test 2"), 1, false);
        }
        await unitTest.delay(DELAY);
        ...
        receivedTopics = {};
        for (let i = 1; i <= 2; i++) {
            publish.publish(new Message("/a/0/retain", i, "test 1"), 0, true);
            publish.publish(new Message("/a/1/retain", i, "test 2"), 1, true);
            publish.publish(new Message("/a/2/retain", i, "test 2"), 1, false);
            publish.publish(new Message("/b/2/retain", i, "test 2"), 1, true);
        }
        ...
        // Case 1: empty string
        publish.publish({topic:"/a/0/retain", value:""}, 0, true);
        // Case 2: non empty string
        publish.publish({topic: "/a/1/retain", value: 1}, 1, true);
        // Case 3: does not have a retain message
        publish.publish({topic: "/a/2/retain", value: ""}, 1, true);
        // Case 4: fully scip the value
        publish.publish({topic: "/b/2/retain"}, 1, true);
        receivedTopics = {};
        await disconnectBroker();
        await connectToBroker(true);
        await unitTest.delay(1000);
        ...
        publish.publish({topic: "/a/1/retain", value: ""}, 1, true);
        ...
        await disconnectBroker();
        for (let i = 1; i <= LAST; i++) {
            publish.publish(new Message("/a/0/disconnect", i, "test 1"), 0, false);
            publish.publish(new Message("/a/1/disconnect", i, "test 2"), 1, false);
        }
        await unitTest.delay(1000);
        await connectToBroker(false);
        await unitTest.delay(DELAY);
        ...
        await receive.close();
        for (let i = 1; i <= LAST; i++) {
            publish.publish(new Message("/a/0/close", i, "test 1"), 0, false);
            publish.publish(new Message("/a/1/close", i, "test 2"), 1, false);
        }
        ...
        await receive.close();
        for (let i = 1; i <= LAST; i++) {
            publish.publish(new Message("/a/1/auto disconnect", i, "test 2"), 1, false);
        }
