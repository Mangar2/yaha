 ...
 module.exports = class CheckInput {
    ...
    static anyOf(definitions, variable) {
        ...
        for (let definition of definitions) {
            let current = CheckInput.checkType(definition, variable);
            if (current === true) {
                result = true;
                break;
            }
    ...
    static checkProperties(propertyDefintions, object) {
        ...
        if (propertyDefintions !== undefined) {
            for (let property in propertyDefintions) {
                if (object[property] !== undefined) {
                    let check = CheckInput.checkType(propertyDefintions[property], object[property]);
                    if (check !== true) {
    ...
    static checkObject(definition, object) {
        ...
        if (typeof(object) !== 'object') {
            ...
            let required = CheckInput.checkRequired(definition.required, object);
            let properties = CheckInput.checkProperties(definition.properties, object);
    ...
    static checkArray(definition, array) {
        ...
        if (!Array.isArray(array)) {
            ...
            for(let index in array) {
                ...
                let check = CheckInput.checkType(definition.items, item);
                if (check !== true) {
    ...
    validate(data) {
        let result;
        let check = CheckInput.checkType(this.definition, data);
        if (check === true) {
            result = true;
            this.messages = "";
        } else {
    ...
    throwOnValidationError(data, message) {
        if (!this.validate(data)) {
            throw(message + JSON.stringify(this.getMessages()));
        }
    }
 }