...
class Connect {
     ...
     * Gets/Sets token
     * @type {Object} token
     * @property {string} send send token
     * @property {string} receive receive token
     */
     ...
     */
    async _send (path, headers, payload) {
     ...
     * @param {number} keepAlive time in milliseconds the broker will keep the connection without receiving calls
     * @returns {Object} payload returned from connect request
     */
    async connect (clean, version = '1.0', keepAlive) {
        const httpObjects = mqttVersion.connect(version, {
     ...
     */
    async connectAndSubscribe (clean, topics, version = '1.0', keepAlive) {
        let result
        result = await this.connect(clean, version, keepAlive)
        if (this.isConnected) {
