...
module.exports = class Connect {
    ...
    async connect (clean, version = '1.0') {
        const httpObjects = mqttVersion.connect(version, this.clientId, this.myAddress, this.listenerPort, clean)
        const result = await this.putJsonMessage('/connect', httpObjects.headers, httpObjects.payload)
    ...
    async disconnect (version = '1.0') {
        const httpObjects = mqttVersion.disconnect(version, this.clientId)
        const result = await this.putJsonMessage('/disconnect', httpObjects.headers, httpObjects.payload)
    ...
    async subscribe (topics, version = '1.0') {
        const httpObjects = mqttVersion.subscribe(version, topics, this.clientId, this.providePacketId())

        const result = await this.putJsonMessage('/subscribe', httpObjects.headers, httpObjects.payload)
    ...
    async unsubscribe (topics, version = '1.0') {
        const httpObjects = mqttVersion.unsubscribe(version, topics, this.clientId, this.providePacketId())
        const result = await this.putJsonMessage('/unsubscribe', httpObjects.headers, httpObjects.payload)
