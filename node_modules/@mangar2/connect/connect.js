...
module.exports = class Connect {
    ...
    async connect(clean, version) {
        ...
        if (!httpObjects.resultCheck(result.statusCode, result.headers, result.payload)) {
     ...
     * @param {string | undefined} version interface version (0.0 or 1.0, default is 1.0)
     */
    async subscribe(topics, version) {
        version = version === undefined ? '1.0' : version;
        let httpObjects = mqttVersion.suscribe(version, topics, this.clientId, this.providePacketId());
        
        let result = await this.putJsonMessage("/subscribe", httpObjects.headers, httpObjects.payload);
        let headers = result.headers;

        if (!httpObjects.resultCheck(result.statusCode, headers, result.payload)) {
     ...
     * @param {string | undefined} version interface version (0.0 or 1.0, default is 1.0)
     */
    async unsubscribe(topics, version) {
        let payload = {
            clientId: this.clientId,
            topics,
            version
     ...
     * @param {string | undefined} version interface version (0.0 or 1.0, default is 1.0)
     */
    async connectAndSubscribe(clean, topics, version) {
        let result;
        await this.connect(clean, version);
        if (this.isConnected) {
            result = await this.subscribe(topics, version);
        }
        return result;
    }
