 ...
 module.exports = class Client {
    ...
    publishMessage(message, qos, dup, pattern) {
        ...
        if (sendQos !== undefined) {
            ...
            let messageQueueEntry = new MessageQueueEntry(sendQos, message);
            if (sendQos === 0) {
    ...
    _getOrderedMessagesToSend(topic, inFlightWindow, timeoutInMilliseconds) {
        ...
        let result = [];
        for (let entry of this.orderedTopicsQueue[topic]) {
            if (entry.status === "new") {
    ...
    _getNotOrderedMessagesToSendAndClearTheQueue() {
        let  result = [];
        if (this.isConnected()) {
            for (let entry of this.QoS0Queue) {
                entry.status = "sending";
                entry.setClientInfo(
                    this.clientId, this.host, this.port, this.version, this.token.receive);
                result.push(entry);
    ...
    getAllMessagesToSend(inFlightWindow, timeoutInMilliseconds) {
        ...
        let notOrderedMessages = this._getNotOrderedMessagesToSendAndClearTheQueue();
        if (this.isConnected()) {
            for (let topic in this.orderedTopicsQueue) {
                let messages = this._getOrderedMessagesToSend(
