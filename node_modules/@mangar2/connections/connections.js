...
module.exports = class Connections {
    constructor (configuration) {
        ...
        this.tokenMap = {}
        this.on = {
            password: (username, password) => { return true },
    ...
    connect (clientId, host, port, clean = true, version = '1.0') {
        ...
        const token = client.connect(clientId, host, port, clean, version)
    ...
    subscribe (clientId, topics) {
        ...
        if (topics.topics === undefined) {
            ...
            for (const retainTopic in this.retainMessages) {
                ...
                const message = this.retainMessages[retainTopic].message
     ...
     * @param {string} topic topic sent
     * @param {number} packetid packet number
     */
    acknowledgeMessage (clientId, topic, packetid) {
        const client = this.getClientById(clientId)
        client.acknowledgeMessageById(topic, packetid)
    }
     ...
     */
    getClientMessagesToSend (client) {
        const messages = client.getAllMessagesToSend(
            this.configuration.inFlightWindow,
            this.configuration.timeoutInMilliseconds)
    ...
    getAllMessagesToSend () {
        ...
        for (const clientId in this.clients) {
            ...
            const clientMessages = this.getClientMessagesToSend(client)
