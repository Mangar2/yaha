...
let lastId = {}
async function sendAllMessages(publishExpected, pubrelExpected, errorText, debug) {
    let receivedMessages = {publish:0, pubrel:0}
    connections.sendAllMessages(async (options) => {
        let {host, port, packet, token, message, qos, packetid} = options
        if (debug) console.log(options);
        receivedMessages[packet]++
...
(async () => {    
    ...
    subscribe()
    queueMessages()
    await sendAllMessages(6, 1, "test subscribed")
    await sendAllMessages(0, 0, "test subscribed, messages sent")
    ...
    queueMessages()
    disconnect()
    await sendAllMessages(0, 0, "test disconnected")
    connect()
    // client2 is clean, ony client 1 packages will be received
    await sendAllMessages(2, 1, "test reconnect")
    ...
    await sendAllMessages(2, 1, "test unsubscribe")
    ...
    await sendAllMessages(0, 0, "in retry timeout")
    await unitTest.delay(15)
    // after a timeout delay sending messages works ...
    for (let i = 0; i < 10; i++) {
        await sendAllMessages(3, 1, "after retry timeout")
    }
    // Check that all messages are processed
    await unitTest.delay(15)
    await sendAllMessages(0, 0, "no message left")
    ...
    await sendAllMessages(3, 1, "not yet disconnected")
    queueMessages()
    for (let i = 0; i <= 8; i++) {
        sendAllMessageNotReturningId()
        await unitTest.delay(15)
    ...
    await sendAllMessages(0, 0, "auto disconnected")
    connect()
    // Two messages, because client1 is clean after connect
    await sendAllMessages(2, 1, "reconnect after auto disconnect")
    ...
    await sendAllMessages(2, 0, "retain, receive message with empty value")
    queueMessages(RETAIN)
    await sendAllMessages(6, 1, "retain, receive messages normally")
    await sendAllMessages(0, 0, "retain, check no message left to send")
    subscribe()
    await sendAllMessages(6, 1, "retain, message received due to retain")
    disconnect()
    connect()
    await sendAllMessages(0, 0, "retain, no messages after reconnect")
    connections.publishMessage(new Message("topic/0", "", "reason0"), 0, RETAIN)
    ...
    await sendAllMessages(3, 0, "retain, send empty retain messages, one client is clean")
    subscribe()
    await sendAllMessages(0, 0, "retain, all retain messages deleted")
    ...
    await sendAllMessages(6, 1, "persisted")
    await sendAllMessages(0, 0, "all messages sent")
    disconnect()
    await connections.restoreFromFile()
    await sendAllMessages(6, 1, "restored")
    
    await cleanFiles()

    unitTest.showResult(229)
