...
async function sendAllMessages(expectedMessages, errorText) {
    ...
    connections.sendAllMessages(async (host, port, message, headers) => {
        if (unitTest.verbose) {
            console.log("host %s, port %s, message %s, QoS %s, id %s", host, port, JSON.stringify(message), headers.qos, headers.packetid);
        }
        ...
        if (headers.packetid != undefined && _lastId !== undefined) {
            unitTest.assertTrue(headers.packetid >= _lastId, "topic: " + topic + " " + headers.packetid + " >= " + _lastId, false);
        }
        lastId[host][topic] = headers.packetid;
        receivedMessages++;
        if (headers.qos > 0) {
