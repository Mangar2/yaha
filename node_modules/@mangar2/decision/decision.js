...
class Decision {
    ...
    _evalOperator (jsonNode) {
        ...
        const { operator, operands } = jsonNode
        if (!types.isArray(operands)) {
            throw Error('operator without operands array ' + JSON.stringify(jsonNode))
        }
        if (operands.length !== 2) {
            throw Error('operands array needs exactly two elements' + JSON.stringify(jsonNode))
        }
        const first = this._getValue(operands[0])
        const second = this._getValue(operands[1])
