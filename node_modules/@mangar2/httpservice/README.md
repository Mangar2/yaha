...
`send (path, method, payload, headers) => {object}`
...
| `headers` | `object` | header to send | |

##### send returns

| Type | Description |
| ---- | ----------- |
| `object` | Promise; resolve = { statusCode , headers , payload } |
...
| Name | Type | Description |
| ---------- | ------------ | ----------------- |
| `event` | `string` | RESTful http verb ( POST , GET , PUT , PATCH , DELETE ) and 'listen' ( called once listening ) and 'closed' ( called once closed ) | |
