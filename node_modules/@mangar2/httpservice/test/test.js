...
const HttpServer = require('@mangar2/httpserver')
const errorLog = require('@mangar2/errorlog')
const shutdown = require('@mangar2/shutdown')

const client = new HttpClient('127.0.0.1', 9001)

const server = new HttpServer(9001)
...
(async () => {
    const header = { 'Content-Type': 'application/json', packetId: 1, mqttpacket: 'publish' }
    const result = await client.send('/clients', 'GET', 'test', header).catch(err => {
        errorLog(err)
        process.exit(1)
    })
    console.log(result)
