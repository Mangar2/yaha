...
class Parse {
    ...
    _parseComment () {
        ...
        while (!isDocEndTag(this._tokenizer.token)) {
            ...
            if (isDocTag(tk)) {
                ...
                case 'property': result.property = this._parseParam(result.property); break
                ...
                case 'typedef': result = this._parseTypedef(this._tokenizer, result); break
     ...
     * @param {string[]} properties array of properties to look for
     * @returns {Object} modified parent
     */
    _addToParent (element, parent, properties) {
        for (const property of properties) {
            if (element[property] !== undefined) {
                ...
                parent[property].sort((child1, child2) => {
                    if (child1[property] < child2[property]) return -1
                    if (child1[property] > child2[property]) return 1
                    return 0
                })
            }
        ...
        while (this._tokenizer.token !== '') {
            if (isDocBeginTag(this._tokenizer.token)) {
                ...
                if (fullInfo.class) {
                    ...
                    parent = fullInfo
                } else if (fullInfo.function || fullInfo.typedef || fullInfo.callback) {
                    parseResult = this._addToParent(fullInfo, parseResult, ['function', 'typedef', 'callback'])
                } else if (parent === null) {
