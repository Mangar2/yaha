...
function _parseMethod (tokenizer) {
    const result = { method: '' }
    while (['async', 'static'].includes(tokenizer.token)) {
        if (tokenizer.token === 'async') {
            result.async = 'async'
            tokenizer.nextToken()
        }
        if (tokenizer.token === 'static') {
...
function _parseMember (tokenizer) {
    tokenizer.nextToken()
    const result = { name: tokenizer.token }
    tokenizer.nextToken()
    return result
}
...
function parseJS (tokenizer, inClass) {
    ...
    case 'set': result.member = _parseMember(tokenizer); break
    case 'get': result.member = _parseMember(tokenizer); break
    default:
        if (inClass) {
