...
/**
 * @private
 * Decompresses an entry of type single
 ...
 * @param {Reason} [reason] reason of the entry
...
/**
 * @private
 * Decompresses an entry of type timevalue
 * @param {object} entry entry to decompress
...
/**
 * @private
 * Decompresses an entry of type time
 * @param {object} entry entry to decompress
 * @param {number} entry.value value of the entry
...
/**
 * @private
 * Decompresses an entry of type interval - creating "amount" of separate entries
 * @param {object} entry entry to decompress
 * @param {number} entry.value value of the entry
...
function decompressInterval (entry) {
    let remainingAmount = entry.amount - 1
    const value = entry.value
    let time = entry.firstTime
    const result = [{ time: new Date(time).toISOString(), value, reason: entry.reason }]

    while (remainingAmount > 0) {
        const interval = (entry.lastTime - time) / remainingAmount
        time += interval
        remainingAmount--
        result.unshift({ time: new Date(time).toISOString(), value })
    }
 ...
 * @param {[[number, number]]} [history[].values] array of [time, value] (only type 'timeValue')
