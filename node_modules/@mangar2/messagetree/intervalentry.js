...
function getLastIntervalEntries (timeEntry, options) {
    ...
    if (entryAmount <= options.lengthForFurtherCompression) {
        return result
    }
    result.firstTime = timeEntry.timestamps[entryAmount - options.lengthForFurtherCompression]
    const joinedInterval = (result.lastTime - result.firstTime) / (options.lengthForFurtherCompression)
