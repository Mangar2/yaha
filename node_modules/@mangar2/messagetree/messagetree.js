...
module.exports = class MessageStore {
    ...
    setFromConfiguration (propertyName, options, defaultValue) {
        this.options[propertyName] = Number(options[propertyName]) || defaultValue
    }
    ...
    static updateContent (node, content, options) {
        if (Object.hasOwnProperty.call(node, 'value')) {
            ...
            node.history = addToHistory(historyEntry, node.history, options)
        }
    ...
    static getSectionRec (node, topic, levelAmount) {
        ...
        if (levelAmount > 0) {
            for (const childName in node.childs) {
                ...
                const subChildNodes = this.getSectionRec(childNode, childTopic, levelAmount - 1)
                result = {
    ...
    getSection (topic, levelAmount) {
        ...
        const result = MessageStore.getSectionRec(node, topic, levelAmount)
        return result
    }
}
