...
class MessageTree {
     ...
     */
    _getPayload (node, addHistory = true) {
        const result = {
            topic: node.topic,
     ...
     */
    _getSectionRec (node, topic, levelAmount, addHistory) {
        ...
        if (node !== undefined) {
            if (this._isDataNode(node)) {
                result.payload[topic] = this._getPayload(node, addHistory)
            }
            ...
            if (levelAmount > 0) {
                for (const childName in node.childs) {
                    ...
                    const subChildNodes = this._getSectionRec(childNode, childTopic, levelAmount - 1, addHistory)
                    result = {
    ...
    getSection (topic, levelAmount, addHistory) {
        ...
        const result = this._getSectionRec(node, topic, levelAmount, addHistory)
        return result
    }
}
