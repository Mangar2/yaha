...
module.exports = class MessageStore {
     ...
     * @param {object} options object with property propertyName
     ...
     * @param {number} options.lowerBoundFactor0.8, lower bound factor to compare time spans
    ...
    readTreeFromPersistedFile (directory, filenameBasis) {
        this.tree = persist.readData(directory, filenameBasis)
    }
    ...
    addData (message) {
        ...
        MessageStore.updateContent(node, content, this.options)
    };
    ...
    static getSectionRec (node, topic, levelAmount, addHistory) {
        ...
        if (node !== undefined) {
            if (MessageStore.isDataNode(node)) {
                result.payload[topic] = MessageStore.getPayload(node, addHistory)
            }
     ...
     * @param {boolean} [addHistory=true] true, if the history property will be added
     * @returns {{topics, payload}} list of (sub)topics and stored messages with history
     * for these (sub)topics
     */
    getSection (topic, levelAmount, addHistory) {
        const node = this.getNode(topic)
        const result = MessageStore.getSectionRec(node, topic, levelAmount, addHistory)
        return result
    }
}
