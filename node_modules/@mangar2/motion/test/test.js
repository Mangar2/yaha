...
function runTest (testcase) {
    const motion = new Motion(testcase.rule, { motionTopics: ['+/+/motion'] })
    const subscriptions = motion.getSubscriptions(2)
    unitTest.assertDeepEqual(subscriptions, { '+/+/motion': 2, 'system/state/presence': 2 })
    ...
    for (const test of testcase.tests) {
        ...
        for (const index in test.results) {
            ...
            if (message !== undefined) {
                unitTest.assertEqual(expected.topic, message.topic, test.name)
                unitTest.assertEqual(expected.value, message.value, test.name)
            }
        }
    }
}
