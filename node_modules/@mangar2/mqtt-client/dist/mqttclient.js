...
class MqttClient {
        ...
        this._server.on('publish', async (message) => {
            for (const key in this._recipients) {
                ...
                if (topicMatch.getFirstMatch(message.topic) !== undefined) {
                    const messageCopy = new mqtt_utils_1.Message(message.topic, message.value, message.reason);
    ...
    _createMemoryUsageMessage() {
        ...
        const reason = 'rss: ' + memory.rss / BYTE_TO_MBYTE + 'MB ' +
        ...
        const message = new mqtt_utils_1.Message('$SYS/' + this._clientId + '/memory usage', memory.rss / BYTE_TO_MBYTE, reason);
        message.qos = 0;
        return message;
    }
    /**
