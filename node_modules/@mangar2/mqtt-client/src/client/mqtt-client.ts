...
import {
    Message, IMessage,
    TopicMatch, Logger, LogPattern, createMemoryUsageMessage,
    SubscribeResult,
    topics_t
} from '@mangar2/mqtt-utils';
import { BrokerOptions, HttpReceiveServices, HttpSendServices } from '@mangar2/mqtt-client/src';
import { IMqttClient, receipientCallback_t, pollCallback_t } from '@mangar2/mqtt-client/src/registration/imqtt-client';
...
export class MqttClient implements IMqttClient {
    ...
    async subscribe(serviceName: string, topics: topics_t): Promise<SubscribeResult> {
        ...
        if (!this.isShuttingDown) {
            ...
            if (this.connected) {
                return await this._client.connect.subscribe(this._version, topics)
            }
        }
        return [127];
    }
    ...
    async publish(message: IMessage, serviceName: string = ''): Promise<string[]> {
        console.log(`publishing message ${message}, service name: ${serviceName}`);
        const token = this._token ? this._token.send : 'undefined'
        return this._client.publish(token, message, this._version)
    }
