...
import { Message, topics_t } from '@mangar2/mqtt-utils';
import { BrokerOptions, HttpReceiveServices, HttpSendServices } from './index';
import { LogPattern, createMemoryUsageMessage } from '@mangar2/mqtt-utils';
...
export class MqttClient {
    ...
    private _brokerOptions: BrokerOptions;
    private _clientId: string;
    private _clean: boolean;
    private _connected: boolean;
    private _version: '0.0' | '1.0';
    private _isShuttingDown: boolean = false;
    private _token: { send: string, receive: string };
    private _callbacks: Callbacks;
    private _closeChain: (() => void)[];
    private _recipients: { [key: string]: { callback: (message: Message) => Message | Message[], topics: topics_t } } = {}
    ...
    constructor(options: MqttClientOptions) {
        ...
        this._server = new HttpReceiveServices(listener, logPattern)
        this._client = new HttpSendServices(clientId, brokerOptions)
     ...
     */
    async updateSubscriptions (serviceName: string, topics: topics_t) {
    ...
    publish (message: Message) {
        this._server.logFilter.condLogMessage('send', message, message.qos, false, this._clientId)
     ...
     */
    _initCallbacks () {
        this._server.onPublish.on('publish', async (message: Message) => {
            ...
            try {
                ...
                if (memoryUsageMessageCountdown <= 0) {
                    if (memoryUsageTicks > 0) {
                        this.publish(createMemoryUsageMessage())
                    }
