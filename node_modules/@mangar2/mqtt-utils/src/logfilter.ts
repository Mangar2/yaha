...
import { IMessage } from './message';  //Assuming there is a message class somewhere in your project
...
export class LogFilter {
     ...
     * @param {boolean} retain - Retain message flag
     * @param {string} [serviceName] - Name of the service logging. Default value is an empty string.
     */
    condLogMessage(moduleStr: string, message: IMessage, dup: boolean, retain: boolean, serviceName = ''): void {
        const { topic, value, reason, qos } = message;
        ...
        if (logLevel) {
            ...
            console.log(`${new Date().toLocaleString()} ${serviceName} (qos${qos}${dupStr}${retainStr}) ${moduleStr} ${topic}${valueStr} [${reasonStr}]`);
        }
    }
}
