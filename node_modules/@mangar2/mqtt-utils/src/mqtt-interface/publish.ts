...
import { IResult, IResultWithPacketid, RequestData, standardHeaderJSON, headers_t } from "./interfaces"
...
interface PublishFunction {
    (options: IPublishOptions): RequestData;
}
...
export const publish: Record<string, PublishFunction> = {
    '0.0': (options: IPublishOptions): RequestData => {
        /**
         ...
         */
        const resultCheck = (result: IResult): boolean => {
            return Number(qos) === 0 || (
    ...
    '1.0': (options: IPublishOptions): RequestData => {
        /**
...
export const onPublish: Record<string, PublishResponseFunction> = {
    '0.0': (headers: headers_t): IResult => {
         ...
         * @param {headers_t} headers The headers received from the publish request.
         * @returns {IResultWithPacketid} The result of the publish response.
         */
        const version = '0.0';
    ...
    '1.0': (headers: headers_t): IResult => {
        ...
        const result: IResultWithPacketid = {
            headers: { 'content-type': 'application/json; charset=UTF-8', version, packetid },
            payload: '',
            statusCode: 204,
            packetid: Number(packetid)
        }
