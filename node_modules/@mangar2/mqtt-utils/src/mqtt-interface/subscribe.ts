...
interface SubscribeRequestFunction {
    (topics: Topic, clientId: string, packetid: number): RequestData
}

/** Function type for Subscribe response */
interface SubscribeResponseFunction {
    (headers: Record<string, string>, qosArray: number[]): Result;
}
...
export const subscribe: Record<string, SubscribeRequestFunction> = {
    '0.0': (topics: Topic, clientId: string): RequestData => {
    ...
    '1.0': (topics: Topic, clientId: string, packetid: number): RequestData => {
        const payload = {
            clientId, topics
        }
...
export const onSubscribe: Record<string, SubscribeResponseFunction> = {
    '0.0': (headers: Record<string, string>): Result => {
        return {
    ...
    '1.0': (headers: Record<string, string>, qosArray: number[]): Result => {
