 ...
 */
export type SubscribeReturnCodes = 0 | 1 | 2 | 80;
    ...
    '1.0': (topics: topics_t, clientId: string, packetid: number): RequestDataV2 => {
        ...
        const resultCheck = (result: IResult) => {
            ...
            if (!Types.isArray(payload)) {
                throw new Error('subscribe result has illegal format, array expected');
            }
            if (payload.every(code => [0, 1, 2, 80].includes(code))) {
                throw new Error(`illegal return codes in subscribe result`);
            }
