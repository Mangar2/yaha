...
import { IResult, RequestData, standardHeaderJSON, standardHeaderText, topics_t } from "./interfaces"
 ...
 */
const unsubscribe: Record<string, (topics: topics_t, clientId: string, packetid: number) => RequestData> = {
    '0.0': (topics, clientId) => {
        const payload = { topics, clientId };
        const headers = { ...standardHeaderText, version: '0.0' };

        const resultCheck = (result: IResult) => result.statusCode === 200
    ...
    '1.0': (topics, clientId, packetid): RequestData => {
        ...
        const resultCheck = (result: IResult) => result.statusCode === 204
                && Types.isString(result.headers['content-type']) 
 ...
 */
const onUnsubscribe: Record<string, (headers: Record<string, string>) => IResult> = {
    '0.0': headers => ({
