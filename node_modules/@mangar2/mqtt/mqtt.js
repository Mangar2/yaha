...
module.exports = class Mqtt {
    ...
    onPublish(payload, headers) {
        ...
        let messageObj = new Message(message.topic, Mqtt.parseValue(message.value), message.reason);
        let isDuplicate = false;
        messageObj.addReason("received by broker");
        
        if (headers.version !== undefined && headers.version !== '0.0') {
            let clientId = this.connections.checkToken(token);
    ...
    async _sendPublish(options, callback) {
        let {host, port, version, token, message, qos, dup, packetid} = options;
        let dupStr = dup ? ",dup" : "";
        let valueStr = message.value === "" || message.value === undefined ? "" : "=" + message.value;
        let retain = false;
        let publishData = mqttVersion.publish(version, token, message, qos, dup, retain, packetid);
        let success = false;

        try {
            let result = await callback(host, port, "publish", publishData.payload, publishData.headers);
    ...
    async _sendPubrel(options, callback) {
        let {host, port, version, token, packetid} = options;
        let publishData = mqttVersion.pubrel(version, token, packetid);
        let success = false;

        try {
            let result = await callback(host, port, "pubrel", publishData.payload, publishData.headers);
            success = publishData.resultCheck(result)
        }
        catch (err) {
            if (this.logFilter.getLogLevel("send", "system/broker/error") !== undefined) {
    ...
    processSendMessage(callback) {
        return this.connections.sendAllMessages(async (options) => {
            let success = false;
            if (options.packet === "publish") {
                success = await this._sendPublish(options, callback);
            } else if (options.packet === "pubrel") {
                success = await this._sendPubrel(options, callback);
            } else {
                throw Error("Illegal packet type " + options.packet);
            }
            return success;
        });
    }
