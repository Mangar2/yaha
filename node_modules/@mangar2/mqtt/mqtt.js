...
module.exports = class Mqtt {
    ...
    invokeCallback (event, ...param) {
        ...
        if (this.callbacks[event] !== undefined) {
            result = this.callbacks[event](...param)
        }
        return result
    }
    ...
    onConnect (payload, headers) {
        ...
        try {
            if (!this.invokeCallback('login', user, password)) {
            ...
            const message = new Message('$SYS/broker/connect/success', 1, 'request from ' + payload.clientId)
            this.connections.setWill(clientId, will)

            this.publish(message, 1, false, false)
        } catch (err) {
            const message = new Message('$SYS/broker/connect/refused', 0, err.message)
            this.publish(message, 1, false, false)
            throw err
        }
        return result
    ...
    onDisconnect (payload, headers) {
        ...
        this.publish(message, 1, false, false)
        const result = mqttVersion.onDisconnect(headers)
     ...
     */
    onPingreq (payload) {
        const clientId = this.connections.getClientByToken(token).getClientid();
        const message = new Message('$SYS/broker/ping', '', 'request from ' + payload.clientId)
        this.publish(message, 1, false, false)
     ...
     * @param {function} callback (host, port, payload, headers)
     * @returns {boolean} true, if sending was successful
     */
    async _sendPublish (options, callback) {
        const { host, port, version, token, payload, qos, packetid } = options
        const dup = options.getDup()
        const dupStr = dup ? ',dup' : ''
        const valueStr = payload.value === '' || payload.value === undefined ? '' : '=' + payload.value
        const retain = false
        const publishData = mqttVersion.publish(version, token, payload, qos, dup, retain, packetid)
        let success = false

        try {
            const result = await callback(host, port, 'publish', publishData.payload, publishData.headers)
            success = publishData.resultCheck(result)

            if (this.logFilter.getLogLevel('send', payload.topic) !== undefined) {
                const reason = payload.reason === undefined ? '' : payload.reason[0].message
                console.log('%s %s:%s (qos%s%s) send %s=%s [%s]', new Date().toLocaleString(), host, port, qos, dupStr, payload.topic, valueStr, reason)
            }
        } catch (err) {
     ...
     * @returns {boolean} true, if sending was successful
     */
    async _sendPubrel (options, callback) {
        const { host, port, version, token, packetid } = options
        const publishData = mqttVersion.pubrel(version, token, packetid)
        let success = false

        try {
            const result = await callback(host, port, 'pubrel', publishData.payload, publishData.headers)
            success = publishData.resultCheck(result)
        } catch (err) {
            if (this.logFilter.getLogLevel('send', '$SYS/broker/error') !== undefined) {
     ...
     * @param {function} callback (host, port, payload, headers) function processing the messages
     * @returns {boolean} amount of messages sent
     */
    async processSendMessage (callback) {
        const messages = this.connections.getAllMessagesToSend()
        for (const message of messages) {
            let success = false

            if (message.isPubrel()) {
                success = await this._sendPubrel(message, callback)
            } else {
                success = await this._sendPublish(message, callback)
            }
            if (success) {
                this.connections.acknowledgeMessage(message)
            }
        }
    }
}
