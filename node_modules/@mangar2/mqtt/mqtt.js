...
module.exports = class Mqtt {
    constructor (configuration) {
        ...
        this.on('send', (host, port, path, payload, headers) => {})
    };
     ...
     */
    onPingreq (payload) {
        this.connections.getClientByToken(payload.token).actualizeLastActiveTimestamp()
    ...
    processRequest (controls, payload, headers) {
        let result = { headers: { 'content-type': 'text/plain' }, payload: 'not found ' + controls, statusCode: 404 }
        try {
            switch (controls) {
    ...
    async _sendPublish (options) {
        ...
        try {
            const result = await this.invokeCallback('send', host, port, 'publish', publishData.payload, publishData.headers)
    ...
    async _sendPubrel (options) {
        ...
        try {
            const result = await this.invokeCallback('send', host, port, 'pubrel', publishData.payload, publishData.headers)
            success = publishData.resultCheck(result)
            if (success) {
                this.connections.acknowledgeMessage(options)
            }
