...
module.exports = class mqttControls {
    ...
    publish(message, qos, dup, retain) {
        if (qos === undefined) { throw Error("publish without qos"); }
    ...
    onConnect(payload) {
        ...
        let version = payload.version === undefined ? "0.0" : payload.version;
    ...
    processRequest(controls, payload, headers) {
        ...
        try {
            switch(controls) {
                ...
                case "connect": result = this.onConnect(payload); break;
    ...
    processSendMessage(callback) {
        return this.connections.sendAllMessages(async (host, port, message, headers) => {
            ...
            let valueStr = message.value === "" || message.value === undefined ? "" : "=" + message.value;
            try {
                resultHeaders = await callback(host, port, message, headers);
            ...
            return resultHeaders.packetid === undefined ? -1 : parseInt(resultHeaders.packetid);
        });
    }
