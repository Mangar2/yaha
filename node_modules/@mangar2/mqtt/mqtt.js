...
module.exports = class mqttControls {
     ...
     * @returns {object} {Content-Type, body, httpcode}
     */
    setLog(payload) {
        this.logFilter.changePattern(payload);
        let result = { 
            headers: { "Content-Type": "application/json" },
            payload: this.logFilter.moduleFilter,
            httpcode: 200 
        }
        return result;
    }
     ...
     */
    _publish(message, qos, dup, retain) {
        if (qos === undefined) { throw Error("publishMessage without qos"); }
     ...
     * @returns {object} {Content-Type, headers, body, httpcode}
     */
    publishMessage(message, headers) {
        if (message === undefined) {throw Error("publishMessage without parameters"); }
        ...
        this._publish(messageObj, qos, dup, retain);

        let packet = qos == 1 ? "puback" : (qos == 2 ? "pubrec" : "");
        let result = {
            headers: { "Content-Type": "application/json", packet },
            payload: "",
            httpcode: 204
        }
        if (packetid !== undefined && qos > 0) {
     ...
     * @param {object} headers {packetid} packet identifier
     * @returns {object} {Content-Type, headers, body, httpcode}
     */
    subscribe(payload, headers) {
        ...
        let result = {
            headers: {'Content-Type': "application/json", packet: "suback", packetid},
            payload: {QoS},
            httpcode: 200
        }
        return result;
     ...
     * @param {object} payload {clientId, topics}
     * @returns {object} {Content-Type, httpcode}
     */
    unsubscribe(payload) {
     ...
     * @returns {object} {Content-Type, headers, body, httpcode}, headers and body is optional
     */
    processRequest(controls, payload, headers) {
        let result = {'Content-Type': "text/plain", payload: "not found " + controls, httpcode: 404};
        try {
            switch(controls) {
                case "publish": result = this.publishMessage(payload, headers); break;
                case "connect": result = this.connect(payload); break;
                case "disconnect": result = this.disconnect(payload); break;
                case "subscribe": result = this.subscribe(payload, headers); break;
                case "unsubscribe": result = this.unsubscribe(payload); break;
                case "pingreq": result = this.pingreq(payload); break;
                case "log": result = this.setLog(payload); break;
            }
        }
        catch (err) {
            errorLog(err);
            result = {'Content-Type': "application/json", payload:{error: err.message}, httpcode: 400}
        }
        return result;
    }
    ...
    processSendMessage(callback) {
        return this.connections.sendAllMessages(async (host, port, message, headers) => {
            let packetId = -1;
            let dupStr = headers.dup ? ",dup" : "";
            let valueStr = message.value === "" || message.value === undefined ? "" : "=" + message.value;
            try {
                packetId = await callback(host, port, message, headers);
