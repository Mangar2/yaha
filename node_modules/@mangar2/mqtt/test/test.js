...
mqtt.on('send', async (host, port, packet, payload, headers) => {
    ...
    if (packet === 'publish') {
        ...
        if (payload.message.reason[0].message === 'delete retain') {
            ...
            unitTest.assertEqual(message.value, 1, 'message value')
        }
    } else {
...
function testSubscribeUnsubscribeNewFormat () {
    let result = mqtt.processRequest('subscribe',
        { clientId: '/a/b', topics: { topic1: 1, topic2: 0, topic3: 2 } },
        { packetid: 1, version: '1.0' }
    )
    unitTest.assertEqual(JSON.parse(result.payload).qos[0], 1, 'subscribe 1')
    result = mqtt.processRequest('unsubscribe', { clientId: '/a/b', topics: ['topic1', 'topic2', 'topic3'] }, { version: '1.0' })
}
...
async function publish (token) {
    ...
    result = mqtt.processRequest('pubrel', { token }, { packetid: '2', version: '1.0' })
...
(async function testInterface () {
    setLog()
    let token = testConnectClean()
    testDisconnect()
    testSubscribeNotConnected()

    token = testConnectClean()
