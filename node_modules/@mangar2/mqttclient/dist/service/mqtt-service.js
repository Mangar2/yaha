...
const on_connect_1 = require("./on-connect");
const on_publish_1 = require("./on-publish");
class MqttService {
    ...
    processRequest(control, payload, headers) {
        ...
        try {
            switch (control) {
                case 'publish':
                    result = this.onPublish.onPublish(payload, headers);
                    break;
                case 'pubrel':
                    result = this.onPublish.onPubrel(headers);
                    break;
                case 'connect':
                    result = this.onConnect.onConnect(payload, headers);
                    break;
                case 'disconnect':
                    result = this.onConnect.onDisconnect(payload, headers);
                    break;
                case 'subscribe':
                    result = this.onConnect.onSubscribe(payload, headers);
                    break;
                case 'unsubscribe':
                    result = this.onConnect.onUnsubscribe(payload, headers);
                    break;
                case 'pingreq':
                    result = this.onConnect.onPingreq(payload);
                    break;
                case 'log':
                    result = this.onPublish.onLog(payload);
                    break;
            }
        }
        catch (err) {
