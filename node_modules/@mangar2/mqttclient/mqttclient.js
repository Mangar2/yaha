...
class MqttClient {
     ...
     */
    publish (message) {
        this._logFilter.condLogMessage('send', message, message.qos, false)
    ...
    _createMemoryUsageMessage () {
        ...
        const message = new Message('$SYS/' + this._clientId + '/memory usage', reason)
        return message
    }
    ...
    async _keepConnected () {
        ...
        while (!this.isShuttingDown()) {
            try {
                ...
                if (memoryUsageCountDown <= 0) {
                    this.publish(this._createMemoryUsageMessage())
                    memoryUsageCountDown = 100
                }
                memoryUsageCountDown--
            } catch (error) {
                this.connected = false
                errorLog(error)
            }
