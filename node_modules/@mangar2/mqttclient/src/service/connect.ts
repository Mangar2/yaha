...
import { Interfaces, IConnectResult, IConnectOptions, SubscribeResult, UnsubscribeResult, topics_t } from '@mangar2/mqtt-utils';
...
export class Connect {
    ...
    private _token: { send: string, receive: string } | undefined = undefined;
     ...
     */
    async connect(version: string = '1.0', options: IConnectOptions): Promise<IConnectResult> {
        try {
            options.clientId = this.clientId;
            const sendData = Interfaces.connect(version, options);
    ...
    async disconnect(version: string = '1.0'): Promise<void> {
        try {
            const sendData = Interfaces.disconnect(version, this.clientId);
    ...
    async subscribe(version: string = '1.0', topics: topics_t): Promise<SubscribeResult> {
        try {
            const sendData = Interfaces.subscribe(version, topics, this.clientId, this.providePacketId());
    ...
    async unsubscribe(version: string = '1.0', topics: topics_t): Promise<UnsubscribeResult> {
        try {
            const sendData = Interfaces.unsubscribe(version, topics, this.clientId, this.providePacketId());
     ...
     */
    async connectAndSubscribe(version: string = '1.0', topics: topics_t, connectOptions: IConnectOptions): 
        Promise<{connectResult: IConnectResult, subscribeResult: SubscribeResult}> {
        const connectResult = await this.connect(version, connectOptions);
        const subscribeResult = await this.subscribe(version, topics);
