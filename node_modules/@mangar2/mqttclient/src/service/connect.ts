...
export class Connect {
     ...
     */
    async connect(clean: boolean, version: string = '1.0', keepAlive: number = 600 * 1000): Promise<string> {
        let token = ''
        try {
            const sendData = Interfaces.connect(version, {
            ...
            if (version >= '1.0' && (!Types.isString(payload.token) || payload.token === '')) {
    ...
    async disconnect(version: string = '1.0'): Promise<any> {
        ...
        try {
            ...
            throw new Error(`Unable to disconnect, ${(err as Error).message}`);
        }
