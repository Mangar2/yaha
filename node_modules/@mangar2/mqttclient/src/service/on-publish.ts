...
import { Message, Interfaces, LogFilter, LogPattern, IResult } from '@mangar2/mqtt-utils';
...
export class OnPublish {
    ...
    constructor(logSettings: LogSettings, qos2PubrelTimeoutInSeconds: number = 7200) {
     ...
     */
    public handleRequest(payload: string, headers: IncomingHttpHeaders, path: string): IResult {
        switch (path) {
            case '/publish':
                return this.onPublish(JSON.parse(payload), headers);
            case '/pubrel':
     ...
     */    
    onPubrel(headers: IncomingHttpHeaders): IResult { // Define the type for 'res'
     ...
     * @param {ServerResponse} res The server response object.
     */    
    onPublish(payload: any, headers: IncomingHttpHeaders): IResult { 
