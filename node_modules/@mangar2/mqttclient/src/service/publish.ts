...
import { Types, delay } from '@mangar2/utils';
import { HttpClient as Client } from '@mangar2/httpservice';
import { Message, Interfaces } from '@mangar2/mqtt-utils';
...
export class Publish {
     ...
     */
    private async pubrel(packetid: number, token: string, topic: string): Promise<boolean> {
   ...
   /**
     ...
     */
    private async sendMessage(sendData: any): Promise<boolean> {
        ...
        for (let retryCount = 0; retryCount < this.retry; retryCount++) {            
            try {
                ...
                const success = sendData.resultCheck(result);
                if (success) {
                    if (headers.qos === 2) {
                        return await this.pubrel(headers.packetid, payload.token, payload.message.topic);
                    }
                    return true;
                }
            } catch (err) {
        ...
        throw Error('Error sending Message: ' + (sendError? sendError.message: 'Unknown error'));
    }
    ...
    public async publish(token: string, message: Message, version = '1.0'): Promise<string[]> {
        ...
        if (qos === 0) {
            ...
            const sendResult = await this.client.send('/publish', 'PUT', sendData.payload, sendData.headers).catch(() => {
                // Do not care, if send is successful for qos === 0. Still await is needed to safely catch rejects.
            });
            result.push(sendResult && sendResult.statusCode === 204 ? "send (qos 0)": "not send (qos 0)");
        } else {
