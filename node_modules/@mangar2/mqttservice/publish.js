...
module.exports = class PublishMessage {
    ...
    async pubrel(packetid, token, topic) {
        let success = false;
        let retryCount = 0;
        let sendData = mqttVersion.pubrel('1.0', token, packetid);
    ...
    async publish(token, message, qos, retain, version) {
        ...
        if (qos === 0) {
            let sendData = mqttVersion.publish(version, token, message, qos, 0, retain);
            ...
            let sendData = mqttVersion.publish(version, token, message, qos, 0, retain, this.providePacketId());
