...
(async function test() {    
    try {
        ...
        let receive = new MqttService.onPublish(PORT);
        let publish = new MqttService.publish(HOST, PORT, {retry: 2});
        let value = {};
        let amount = 0;
        ...
        for (let i = 1; i <= TEST_COUNT; i++) {
            if (i % 10 == 0) console.log(i + " of " + TEST_COUNT * 2);
            result = await publish.publish(TOKEN, new Message("/a/0", i, "test"), 0, 0);
            result = await publish.publish(TOKEN, new Message("/a/1", i, "test"), 1, 0);
            result = await publish.publish(TOKEN, new Message("/b/1", i, "test"), 1, 0);
            result = await publish.publish(TOKEN, new Message("/a/2", i, "test"), 2, 0);
            result = await publish.publish(TOKEN, new Message("/c/2", i, "test"), 2, 0);
        }
        ...
        console.log("wait for all promises to finish ...");
