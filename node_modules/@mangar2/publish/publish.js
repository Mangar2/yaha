...
module.exports = class PublishMessage {
    ...
    async pubrel(packetid, topic) {
        let success = false;
        let retryCount = 0;
        let sendData = mqttVersion.pubrel('1.0', packetid);

        while (!success && retryCount < this.configuration.retry) {
            this.topicQueues[topic].state = "pubrel";
            try {
                result = await this.client.send("/pubrel", "PUT", sendData.payload, sendData.headers);
     ...
     */
    async publish(message, qos, retain, version) {
        qos = convertQoSToValidQoS(qos);
        retain = retain == 1 || retain === true ? 1 : 0;
        version = version === undefined ? '1.0' : '0.0'
        let result; 
        

        if (qos === 0) {
            let sendData = mqttVersion.publish(version, message, qos, 0, retain);
            await this.client.send("/publish", "PUT", sendData.payload, sendData.headers).catch(err => {
            ...
            let sendData = mqttVersion.publish(version, message, qos, 0, retain, this.providePacketId());
    ...
    async sendMessage(sendData) {
        ...
        if (qos === 2 && success) {
            success = await this.pubrel(headers.packetid, payload.topic);
        }
