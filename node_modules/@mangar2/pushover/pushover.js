 ...
 */
function getReturnMessage (statusCode, messageReason, pushoverResult) {
    let result
    if (statusCode < 300) {
        result = new Message('$SYS/pushover/success', statusCode, messageReason)
        result.addReason('status = ' + pushoverResult.status)
    } else {
        result = new Message('$SYS/pushover/error', statusCode, messageReason)
        result.addReason('status = ' + pushoverResult.status + ' errors = ' + JSON.stringify(pushoverResult.errors))
    }
    return result
}
...
async function publish (message, config) {
    ...
    for (const device of config.devices) {
        ...
        const returnMessage = getReturnMessage(result.statusCode, message.reason, pushoverResult)
