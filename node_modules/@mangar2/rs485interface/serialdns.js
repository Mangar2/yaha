...
class SerialDNS {
    ...
    toMqttMessages (serialMessage, hexString) {
        const clearedHexString = types.isString(hexString) ? hexString.replace(/\[\d+\] /, '') : ''
        const result = this._getMqttMessageUsingSerialMessage(serialMessage, clearedHexString)
        if (result.length === 0) {
            const { command, value } = serialMessage
            const message = new Message(
                this._getTopicUsingSerialMessage(serialMessage),
                this._toMqttValue(command, value),
                'received from arduino: ' + clearedHexString
            )
            result.push(message)
        }
        return result
