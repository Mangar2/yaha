...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: processrule.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: processrule.js</h1>
...
class ProcessRule {
    ...
    _isAllIncluded (topics, events) {
        ...
        if (types.isArray(topics)) {
            ...
            result = events[topics] !== undefined
        }
        return result
    }
    ...
    _checkEvents (rule, events) {
        ...
        if (events !== undefined) {
            result = false
            if (this._isAllIncluded(rule.allOf, events) || this._isAnyIncluded(rule.anyOf, events)) {
                if (!this._isAnyIncluded(rule.noneOf, events)) {
                    result = true
                    if (types.isArray(rule.allow)) {
                        const allow = [...rule.allow]
                        this._addTopics(allow, rule.allOf)
                        this._addTopics(allow, rule.anyOf)
    ...
    /**
     * @private
     * @description Checks if there are any unknown variables
