...
class Rules {
     ...
     * @returns {Object[]} array of invalid rules als { ruleName, messages }
     */
    ...
    invalidateRule (name, messages = []) {
        ...
        for (const rule of rules) {
            if (rule.name === name) {
                this._invalidRules.push({ name, messages })
            } else {
            ...
            for (const key in node) {
                if (key === 'rules') {
                    for (const ruleIndex in node[key]) {
                        ...
                        const isValid = this._check.validate(rule)
                        rule.name = link === '' ? ruleIndex : link + '/' + ruleIndex
                        if (isValid) {
                            this.rules.push(rule)
                        } else {
                            this.invalidRules.push({ name: rule.name, messages: { ...this._check.messages } })
                        }
                    }
                } else {
