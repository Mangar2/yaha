...
class ProcessRule {
    ...
    _checkTime (rule) {
        ...
        const date = this._evalPropertyValue(rule.time)
        const ONE_DAY_IN_SECONDS = 24 * 60 * 60
        if (types.isDate(date)) {
            const duration = this._evalPropertyValue(rule.duration, '6:00')
    ...
    determineNeededVariables (rule) {
        ...
        for (const topic in this._usedVariables) {
            if (this._variables.isExternVariable(topic)) {
                const value = this._variables.getValue(topic)
                result[topic] = value !== undefined ? value : null
            }
        }
    ...
    _getMessagsFromRule (rule, decisionResult) {
        ...
        if (types.isArray(topic)) {
            const value = this._evalPropertyValue(rule.value)
            ...
            for (const oneTopic in topic) {
                const value = this._evalPropertyValue(topic[oneTopic])
                const message = new Message(oneTopic, value, reason, this._date)
                message.qos = qos
                result.messages.push(message)
            }
