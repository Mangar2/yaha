...
class ProcessRule {
    ...
    _checkEvents (rule, events) {
        ...
        if (events !== undefined) {
            result = this._isAllIncluded(rule.allOf, events)
            if (!result.check) {
                result = this._isAnyIncluded(rule.anyOf, events)
            }
            if (result.check && this._isAnyIncluded(rule.noneOf, events).check) {
            ...
            if (result.check && types.isArray(rule.allow)) {
                ...
                if (!this._isSubset(allow, events)) {
                    result.setCheck(false)
                }
            }
        }
    ...
    check (rule, events) {
        ...
        if (this._checkTime(rule)) {
            ...
            if (decisionResult.value) {
                const checkResult = this._checkEvents(rule, events)
