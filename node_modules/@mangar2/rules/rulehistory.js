...
class RuleHistory {
    ...
    _isMessageRedundant (rule, date, message) {
        ...
        let isRedundant = messageFound
        if (messageFound) {
            if (historyEntry.lastSent === undefined) {
                isRedundant = false
            } else if (types.isNumber(rule.cooldownInSeconds)) {
                const cooldown = rule.cooldownInSeconds * MILLISECONDS_IN_A_SECOND
                const needToResend = historyEntry.lastSent.getTime() + cooldown <= date.getTime()
    ...
    _updateHistoryForMessagesToSend (rule, curDate, messages) {
        const { name } = rule
        const lastSend = new Date(curDate.getTime())
    
        for (const message of messages) {
            const { topic } = message
            const ruleHistory = this._history[name]
            ruleHistory[topic].lastSend = lastSend
        }
    }
