...
function runTest (test, testObjects) {
    ...
    const check = process.check(testRules.rules[0], events)
...
testrun.on('validate', (test, result, path) => {
    ...
    testrun.unitTest.assertEqual(testRules.invalidRules.length, 0, 'Invalid test rule(s): "' + path)
    testrun.unitTest.assertEqual(testRules.rules.length, 1, 'Must have exactly one valid rule "' + path)
    testrun.unitTest.assertEqual(check.messages.length, test.messages, 'False result message amount ' + path)
    if (check.messages.length !== test.messages) {
        testrun.runAgain()
    }
...
testrun.unitTest.showResult(188)
