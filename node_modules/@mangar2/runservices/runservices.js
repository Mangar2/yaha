/**
 * @license
 * This software is licensed under the GNU LESSER GENERAL PUBLIC LICENSE Version 3. It is furnished
 * "as is", without any support, and with no warranty, express or implied, as to its usefulness for
 * any purpose.
 *
 * @author Volker Böhm
 * @copyright Copyright (c) 2020 Volker Böhm
 */

'use strict'

const MqttClient = require('@mangar2/mqttclient')
const readConfig = require('./configuration')
const registerMotion = require('./registermotion')
const registerAutomation = require('./registerautomation')

const config = readConfig('yahaconfig.json')
const mqttClient = new MqttClient(config.runservices)

registerAutomation(mqttClient, config)
registerMotion(mqttClient, config)

mqttClient.run()
