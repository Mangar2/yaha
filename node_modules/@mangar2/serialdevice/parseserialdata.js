...
class ParseSerialData {
     ...
     * @returns {SerialMessage|null} object in standard format or null, if receivedObject has an unknown format
    ...
    _transform (receivedObject) {
        ...
        if (types.isArray(receivedObject)) {
            result = new SerialMessage(receivedObject[0], null, receivedObject[1], receivedObject[2])
        } else if (isYahaArduinoFormat) {
            result = new SerialMessage(receivedObject.S, receivedObject.R, receivedObject.K, receivedObject.V)
        } else if (isFS20Format) {
            result = new SerialMessage('FS20', null, receivedObject.Hauscode + '/' + receivedObject.Adresse, receivedObject.Befehl)
        }
        return result
    }
