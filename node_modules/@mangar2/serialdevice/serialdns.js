...
const SerialObject = require('./serialmessage')
...
class SerialDNS {
     ...
     * @param {Object} serialObject serial message to update
     ...
     */
    toSerialObject (mqttMessage) {
        const serialObject = new SerialObject()
        serialObject.sender = this._options.myAddress
        ...
        if (data) {
            serialObject.receiver = data.address
            serialObject.command = data.command
            serialObject.value = data.value
        } else {
            serialObject.receiver = this._getReceiverByTopic(mqttMessage.topic)
            serialObject.command = this._getCommandByTopic(mqttMessage.topic)
            serialObject.value = this._toSerialValue(serialObject.command, mqttMessage.value)
        }
        return serialObject
    }
     ...
     * @param {Object} serialObject received serial message
     * @returns {string} topic
     * @throws {Error} If the topic cannot be derived due to wrong or not configured elements
     * @private
     */
    _getTopicUsingSerialObject (serialObject) {
        const topic =
            this._getTopicBeginUsingSerialAddress(serialObject.sender) +
            this._getTopicEndUsingSerialCommand(serialObject.command)
        return topic
    }

    /**
     * Check, if we can get mqtt messages from the "topics" interface of options
     * @param {Object} serialObject serial message
     * @returns {Message[]} list of mqtt messages to send to broker
     * @private
     */
    _getMqttMessageUsingSerialObject (serialObject) {
        const result = []
        const { command, sender, value } = serialObject
     ...
     * @param {Object} serialObject received serial object
     * @param {string|number} sender sender of the object
     * @param {string} command command of the object
     * @param {string|number} value value of the object
     * @returns {Message[]} Mqtt messages
     * @throws {Error} If the message cannot be derived due to wrong or not configured elements
     */
    toMqttMessages (serialObject) {
        const result = this._getMqttMessageUsingSerialObject(serialObject)
        if (result.length === 0) {
            const { command, value } = serialObject
            const message = new Message(
                this._getTopicUsingSerialObject(serialObject),
