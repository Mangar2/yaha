 ...
 * @param {string} config.user name of the login user
...
class SunnyPortal {
    ...
    async _login () {
        ...
        console.log(success ? 'Login successful' : 'Login not successful')
        return success
    }
    ...
    async processTasks () {
        ...
        for (let count = 0; count < RETRY; count++) {
            data = await this._readProduction()
            if (this._loggedin) {
                break
            }
        }
