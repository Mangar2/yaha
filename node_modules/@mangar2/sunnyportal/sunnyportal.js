...
class MessageList {
     ...
     */
    addMessage (topic, value, timestamp) {
        if (value !== undefined && value !== null && value !== '') {
            this.messages.push(new Message(topic, value, 'read from sunny portal', timestamp))
        }
    }
}
...
class SunnyPortal {
    ...
    async getMessages () {
        ...
        if (this._loggedin && data.Timestamp) {
            ...
            messages.addMessage('outdoor/solar/info', data.InfoMessages.join('; '))
            messages.addMessage('outdoor/solar/warning', data.WarningMessages.join('; '))
            messages.addMessage('outdoor/solar/error', data.ErrorMessages.join('; '))

            messages.addMessage('outdoor/solar/consumption', consumption, timestamp)
            messages.addMessage('outdoor/solar/solar yield', data.PV, timestamp)
            messages.addMessage('outdoor/solar/feedIn', data.FeedIn, timestamp)
            messages.addMessage('outdoor/solar/averageFeedIn', averageFeedIn, timestamp)
        }
