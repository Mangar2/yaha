...
class Testrun {
     ...
     */
    _runTest (test, testPath, validate, callBreak) {
        try {
            if (callBreak) {
                this._callbacks.invokeCallback('break')
            }
            const result = this._callbacks.invokeCallback('run', test)
            if (validate) {
    ...
    _runTestcase (testcase, testcasePath, testNo) {
        for (const test of testcase.tests) {
            ...
            const validate = !(testNo > 0)
            const doBreak = this._testNo === testNo
            this._runTest(test, testPath, validate, doBreak)
    ...
    _runFile (fileName, testNo = null) {
        ...
        for (const testcase of this._fileContent) {
            ...
            this._callbacks.invokeCallback('prepare', testcase, testcasePath)
