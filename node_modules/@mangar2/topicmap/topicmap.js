...
class TopicMap {
    ...
    /**
     * @private
     * Maps a topic to map controlled by a input pattern supporting whildchards in input and map
     * @param {string} topic topic to map
     * @param {string} pattern input pattern
     * @param {string} map output pattern
     */
    _createMapResult (topic, pattern, map) {
        ...
        for (let index = 0; (index < mapChunks.length); index++) {
            ...
            const mapChunk = mapChunks[index]
            if (mapChunk === '#') {
                result += spacer + this._getTopicEnd(topicChunks, patternChunks)
                break
            } else if (mapChunk === '%') {
                result += spacer + topicChunk
            } else {
                result += spacer + mapChunk
            }
            spacer = '/'
        }
        return result
    ...
    /**
     * @private
     * @param {Object} configuration mapping configuration - key: value objects, might be recursive
     */
