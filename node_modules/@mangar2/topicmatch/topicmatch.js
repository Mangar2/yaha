 ...
 * @param {string[]} topicElements topic chunks to check
 * @param {string[]} patternElements topic chunks with wildchards to pattern
 */
function isTopicMatchingSplit (topicElements, patternElements) {
    ...
    let matches = !patternIsLonger && !notMatchingDueToSysTopic(topicElements[0], patternElements[0])
...
function isTopicMatching (topic, pattern) {
    ...
    return isTopicMatchingSplit(topicElements, patternElements)
}
...
class TopicMatch {
     ...
     * @private
     * @deprecated
     * DEPRICATED USE addPattern instead
     ...
     */
    setPattern (pattern, value = 0) { this.addPattern(pattern, value) }
    ...
    addPattern (pattern, value = 0) {
        if (types.isObject(pattern)) {
            this.topicPatternList = { ...this.topicPatternList, ...pattern }
        ...
        for (const index in this.topicPatternList) {
            this._tree.addPattern(index, this.topicPatternList[index])
        }
    }
    ...
    getFirstMatch (searchTopic) {
        ...
        let result
        for (const curPattern in this.topicPatternList) {
            const topicElements = searchTopic.split('/')
            const patternElements = curPattern.split('/')
            if (isTopicMatchingSplit(topicElements, patternElements)) {
                result = this.topicPatternList[curPattern]
                break
            }
        }
        */
        const result = this._tree.getFirstMatch(searchTopic)
        return result
    }
    ...
    getBestMatch (searchTopic, isBetter = (a, b) => { return a > b }) {
        ...
        for (const curPattern in this.topicPatternList) {
            if (isTopicMatching(searchTopic, curPattern)) {
                ...
                if (result === undefined ||
                    (isBetter === undefined && curValue > result) ||
                    (isBetter(curValue, result))) {
                    result = curValue
                }
            }
        }
        result = this._tree.getBestMatch(searchTopic, isBetter)
        return result
    }
}
