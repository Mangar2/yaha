...
module.exports = class TopicStore {
     ...
     */
    static updateData(node, content, maxHistoryLength, historyHysterese) {

        if (node.hasOwnProperty("data")) {
            let entry = {time: node.time, data: node.data};
     ...
     * @param {any} description additional description for the data (optional)
     */
    addData(topic, data, description) {
        let node = this.getAndCreateDataNode(topic);
        let time = new Date().getTime();
        let content = {time, data};
        if (description !== undefined && description !== null) {
            content.description = description;
        }
        TopicStore.updateData(node, content, 
    ...
    static getSectionRec(node, topic, levelAmount) {
        ...
        if (TopicStore.isDataNode(node)) {
            result.payload[topic] = TopicStore.getPayload(node);
    ...
    getSection(topic, levelAmount) {
        ...
        let result = TopicStore.getSectionRec(node, topic, levelAmount);
        return result;
    }
