...
class UnitTest {
     ...
     * @param {string} message message to be displayed on success/error
     * @param {boolean} exact true, if result may not have additional properties
     * @returns {boolean} true, if the test is ok
     */
    validateResult (toBeValidated, expected, message, exact = false) {
        let result = true
        for (const property in expected) {
            if (toBeValidated[property] !== expected[property]) {
                result = false
                this.fail(message + '/' + property)
            }
        }
        if (exact) {
            for (const property in toBeValidated) {
                if (toBeValidated[property] !== expected[property]) {
                    result = false
                    this.fail(message + '/' + property)
                }
