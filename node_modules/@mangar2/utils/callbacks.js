...
class Callbacks {
     ...
     * Yes, it is identical to invokeCallback what is ok, as long as calling the callback is the last action in the method. 
     * I keep it, because if this changes - we would need an incompatible change of the interface
     * @param {string} event - The event name (not case sensitive) for the callback.
     * @param  {...any} params - Any parameters to pass to the callback function.
    ...
    async invokeCallbackAsync (event, ...params) {
        return this.invokeCallback(event, params)
    }
}
