...
class ZwaveController {
    ...
    setValue (topic, value) {
        try {
            ...
            if (device.node_id === undefined) {
                throw Error('device is not a zwave node (missing node_id property in configuration) ' + topic)
            }
            const zwaveObject = getObjectByLabel(this.nodes, objectLabel, device.node_id, device.instance_id)
            if (zwaveObject === undefined) {
                throw Error('zwave control not found (wrong node name) topic: ' + topic 
                    + ' objectLabel ' + objectLabel
                    + ' nodeId: ' + device.node_id + ' instanceId: ' + device.instance_id)
            }
