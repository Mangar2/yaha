 ...
 * @param {number} [device.instance_id = 1] id of the zwave node instance
 ...
 * @returns {{ node_id, instance_id, class_id, type }}
 */
function getZwaveParameters(nodes, label, device) {
    const { topic, node_id, instance_id, class_id, index, type } = device
    ...
    if (label === undefined) {
        const zwaveObject = getObjectByLabel(nodes, label, node_id, instance_id, index)
        class_id = zwaveObject.class_id
        index = zwaveObject.index
        type = zwaveObject.type
    }
    return { node_id, instance_id, class_id, index, type }
}
...
class ZwaveController {
    ...
    registerZwaveCallbacks () {
        ...
        this._zwave.on('notification', (nodeId, notif) => {
            // this.publishValue(nodeid, mapNotification[notif], )
            try {
                const device = this._deviceList.attributeToDevice('node_id', nodeId)
                const topic = device === undefined ? undefined : device.topic
                console.log('notification: %s (%s), %s (%s)', topic, nodeId, mapNotification[notif], notif)
            } catch (err) {
                console.log('notification: %s (%s), %s (%s)', "unknown node", nodeId, mapNotification[notif], notif)
            }
        })
    ...
    setValue (topic, value) {
        try {
            ...
            const zwaveParameters = getZwaveParameters(this.nodes, objectLabel, device)
            if (zwaveParameters.class_id === ZWAVE_CONFIGURATION) {
                this.setConfigParam(zwaveParameters.node_id, zwaveParameters.index, value)
            } else {
                this._setValue(zwaveParameters, value)
            }
        } catch (err) {
            errorLog(err)
        }
