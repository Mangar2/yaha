...
class ZwaveController {
    ...
    registerZwaveCallbacks () {
        this._zwave.on('driver ready', (homeId) => {
            this._homeid = homeId
            console.log('scanning homeid=0x%s...', this._homeid.toString(16))
        })
    ...
    publishValue (nodeId, value, reasonString) {
        try {
            ...
            if (nodeId === 1) {
                ...
                topic = this._zwaveDevices.valueToTopic(value)
            }
            if (topic !== undefined) {
                topic = topic + '/' + label
    ...
    setValue (topic, value) {
        try {
            ...
            const device = this._zwaveDevices.topicToZwaveId(this._nodes, objectLabel, this._deviceTopic)
            if (device === undefined) {
                throw Error('zwave set value with unknown topic ' + topic)
            }
            const zwaveParameters = getZwaveParameters(this._nodes, objectLabel, device)
            if (zwaveParameters.class_id === ZWAVE_CONFIGURATION) {
                this._setConfigParam(zwaveParameters.node_id, zwaveParameters.index, value)
            } else {
