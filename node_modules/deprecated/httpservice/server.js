 ...
 * @private
 * @description
 * Converts req.url into an URL object
 * @param {Object} req https reqest structure
 * @returns {URL} parsed URL element
 */
function getURL (req) {
    // const url2 = new URL.URL(req.url)
    const decodeULR = decodeURI(req.url)
    const url = new URL.URL(decodeULR, 'http://127.0.0.1')
    // const url = URL.parse(decodeULR)
    return url
}
...
class Server {
     ...
     */
    _dispatch (method, payload, req, res) {
        const url = getURL(req)
        const path = url.pathname
        let result
        if (method === 'get') {
            result = this.callbacks.invokeCallback(method, url.searchParams, req.headers, path, res)
        } else {
            result = this.callbacks.invokeCallback(method, payload, req.headers, path, res)
        }
        if (result === null) {
     ...
     * @param {http|https} service to use
     * @param {Object} options additional options
     */
    listen (service, options) {
        this.server = service.createServer(options, (req, res) => {
            try {
                ...
                req.on('end', () => {
                    try {
                        this._dispatch(method, payload, req, res)
                    } catch (err) {
