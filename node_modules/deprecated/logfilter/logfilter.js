...
class LogFilter {
     ...
     * @param {string} [serviceName] name of the service logging
     */
    condLogMessage (moduleStr, message, qos, dup, serviceName = '') {
        if (this.getLogLevel(moduleStr, message.topic) !== undefined) {
            ...
            console.log('%s %s (qos%s%s) %s %s%s [%s]', 
                new Date().toLocaleString(), serviceName, qos, dupStr, moduleStr, message.topic, valueStr, reasonStr)
        }
    }
}
