 ...
 */
function _registerBackgroundTasks (mqttClient, registeredObject, serviceName) {
    if (types.isAnyFunction(registeredObject.on)) {
        registeredObject.on('publish', (message) => mqttClient.publish(message, serviceName))
    }
 ...
 */
function _registerSubscriptions (mqttClient, registeredObject, serviceName, prepare) {
    ...
    mqttClient.registerRecipient(serviceName, subscriptions, async (message) => {
        if (message.topic === updateString) {
            ...
            await prepare(config, registeredObject)
            const newSubscriptions = registeredObject.getSubscriptions ? registeredObject.getSubscriptions() : {}
            newSubscriptions[updateString] = 1
            mqttClient.updateSubscriptions(serviceName, newSubscriptions)
        } else {
 ...
 */
async function registerService (mqttClient, config, serviceName, serviceImplementation) {
    const curConfig = config[serviceName]
    if (curConfig && config.runservices.services.includes(serviceName)) {
        try {
            const { prepare } = require(serviceImplementation)
            // watchRules()
            const registeredObject = await prepare(curConfig)
...
module.exports = { registerService, loadConfiguration }
